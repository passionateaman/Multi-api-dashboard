...
const MultiAPIQueryApp = () => {
  ...
  const normalizeApi2 = (raw) => {
    ...
    // Some backends (like your api2) may return a markdown table string under
    // the `table` key. If so, prefer rendering that as markdown by copying
    // it into `summary` and marking output_type as markdown so the renderer
        ...
        {fullscreenImage && (
          <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="glass-card rounded-2xl border-gold-gradient w-full h-[90vh] flex flex-col overflow-hidden">
              ...
              <div className="flex-1 overflow-auto p-6 bg-slate-900/50 flex items-center justify-center">
                <img src={fullscreenImage} alt="Fullscreen API2" className="max-h-full max-w-full rounded-md" />
              </div>
            </div>
          </div>
        )}
